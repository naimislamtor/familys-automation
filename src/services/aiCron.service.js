...
/**
 * Generate Content & High-Res 1080x1080 Real AI Image Banner for ANY Custom Topic Prompt
 */
async function generateContentAndImageForTopic(userTopicPrompt) {
    ...
    let postText = '';
    const cleanTopic = userTopicPrompt || 'Daily motivational quote for success and happiness';

    if (apiKey) {
        const systemPrompt = `You are an expert social media content creator. Generate an engaging Bengali social media post based on the user topic below.

User Requested Topic: "${cleanTopic}"

Return ONLY a valid JSON object with this field:
1. "postText": A beautifully formatted Bengali social media post with emoji headers, inspiring bullet points, and 4 relevant hashtags.

Example JSON output:
{
  "postText": "✨ **আজকের সুচিন্তা**\\n\\nনতুন দিনে নতুন পথ চলা...\\n\\n#Inspiration #Bangla #Motivation #Success"
}`;
    ...
    // Generate Real 1080x1080 HD AI Photo Banner (Pollinations Real AI Engine)
    const seed = Math.floor(Math.random() * 9000000) + 1000000;
    const promptSlug = encodeURIComponent(`${cleanTopic}, modern aesthetic graphic poster, vibrant cinematic lighting, 4k HD`);
    const mediaUrl = `https://image.pollinations.ai/prompt/${promptSlug}?width=1080&height=1080&nologo=true&seed=${seed}`;

    console.log(`[AI Photo Banner Generator] Successfully created 1080x1080 HD AI Photo for topic: "${cleanTopic}"`);
