...
setInterval(async () => {
    try {
        ...
        const now = new Date();
        const pendingScheduled = posts.filter(p => p.isScheduled && p.status === 'SCHEDULED' && new Date(p.scheduledTime) <= now);

        for (const post of pendingScheduled) {
            console.log(`[Scheduled Post Engine] Executing scheduled post ID: ${post.id}`);
            ...
            db.savePost(post);
            db.addLog('POST', 'ALL', `Scheduled post (ID: ${post.id}) executed automatically!`);
        }
    } catch (err) {
        console.error('[Scheduled Post Engine Error]:', err.message);
    }
}, 30000);
...
// Trigger Daily AI Post Generator immediately for testing
